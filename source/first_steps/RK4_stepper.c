...
void run_rk4(parameters_t *pars) {
	...
	for (size_t nt = 0; nt < Nt - 1; ++nt, t += dt)
	{
		#ifdef ENABLE_FFT_FILTER
			evo_flags.filter = 1;
		#endif
		...
		// k1 & a1
		a1 = mk_velocities(t, field, frw_a[nt], k1, pars);
		#ifdef ENABLE_FFT_FILTER
			evo_flags.filter = 0;
		#endif
		...
		a2 = mk_velocities(t, tmp_k, tmp_a, k2, pars);
		...
		a3 = mk_velocities(t, tmp_k, tmp_a, k3, pars);
		...
		a4 = mk_velocities(t, tmp_k, tmp_a, k4, pars);
