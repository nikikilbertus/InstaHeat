...
Nt = length(a);
field = reshape(field, N(1), N(2), N(3), Nt);
...
alpha(0.5)
subplot(2,3,4)
surf(y,z,log10(squeeze(pl(m1,:,:)))); xlabel('y'); ylabel('z');
zlabel(['log10(' fieldname ')']);
shading interp
subplot(2,3,5)
surf(x,z,log10(squeeze(pl(:,m2,:)))); xlabel('x'); ylabel('z');
zlabel(['log10(' fieldname ')']);
shading interp
subplot(2,3,6)
surf(x,y,log10(squeeze(pl(:,:,m3)))); xlabel('x'); ylabel('y');
zlabel(['log10(' fieldname ')']);
shading interp
...
system(['ffmpeg -i rho3d.avi -vcodec libx264 -filter:v "setpts=15.0*PTS"'...
    filename '.mp4'])
