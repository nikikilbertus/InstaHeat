...
void h5_create_empty_by_path()
{
    ...
    h5_write_compact(H5_MASS_NAME, val, 1, H5D_COMPACT);
    ...
    val[0] = -1.0;
    #endif
    h5_write_compact(H5_INFLATON_MASS_NAME, val, 1, H5D_COMPACT);

    val[0] = pars.dim;
    h5_write_compact(H5_DIMENSION_NAME, val, 1, H5D_COMPACT);

    val[0] = SEED;
    h5_write_compact(H5_SEED_NAME, val, 1, H5D_COMPACT);

    val[0] = pars.file.skip;
    h5_write_compact(H5_STRIDES_TIME_NAME, val, 1, H5D_COMPACT);

    val[0] = RELATIVE_TOLERANCE;
    val[1] = ABSOLUTE_TOLERANCE;
    h5_write_compact(H5_TOLERANCES_NAME, val, 2, H5D_COMPACT);

    val[0] = pars.x.N;
    val[1] = pars.y.N;
    val[2] = pars.z.N;
    h5_write_compact(H5_GRIDPOINTS_INTERNAL_NAME, val, 3, H5D_COMPACT);

    val[0] = SPATIAL_LOWER_BOUND_X;
    val[1] = SPATIAL_UPPER_BOUND_X;
    h5_write_compact(H5_SPATIAL_BOUNDS_X_NAME, val, 2, H5D_COMPACT);

    val[0] = SPATIAL_LOWER_BOUND_Y;
    val[1] = SPATIAL_UPPER_BOUND_Y;
    h5_write_compact(H5_SPATIAL_BOUNDS_Y_NAME, val, 2, H5D_COMPACT);

    val[0] = SPATIAL_LOWER_BOUND_Z;
    val[1] = SPATIAL_UPPER_BOUND_Z;
    h5_write_compact(H5_SPATIAL_BOUNDS_Z_NAME, val, 2, H5D_COMPACT);

    val[0] = pars.x.outN;
    val[1] = pars.y.outN;
    val[2] = pars.z.outN;
    h5_write_compact(H5_GRIDPOINTS_OUTPUT_NAME, val, 3, H5D_COMPACT);

    val[0] = pars.x.stride;
    val[1] = pars.y.stride;
    val[2] = pars.z.stride;
    h5_write_compact(H5_STRIDES_SPACE_NAME, val, 3, H5D_COMPACT);

    val[0] = pars.bunch_davies_cutoff;
    h5_write_compact(H5_BUNCH_DAVIES_CUTOFF_NAME, val, 1, H5D_COMPACT);
    ...
    val[0] = -1.0;
    #endif
    h5_write_compact(H5_MAX_DT_HUBBLE_FRACTION_NAME, val, 1, H5D_COMPACT);
    ...
    val[0] = 0.0;
    #endif
    h5_write_compact(H5_ENABLE_FILTER_NAME, val, 1, H5D_COMPACT);
    ...
    val[0] = 0.0;
    #endif
    h5_write_compact(H5_ENABLE_GW_NAME, val, 1, H5D_COMPACT);
...
void h5_write_followup()
{
    ...
    h5_write_compact(H5_FOLLOWUP_NAME, field, pars.Ntot, H5D_CONTIGUOUS);
}
#endif
