...
static void wrap_up_dopri853()
{
    ...
    h5_write_compact(H5_STEPS_TOTAL_NAME, val, 1, H5D_COMPACT);
    val[0] = (double)dp.n_ok;
    h5_write_compact(H5_STEPS_OK_NAME, val, 1, H5D_COMPACT);
    val[0] = (double)dp.n_bad;
    h5_write_compact(H5_STEPS_BAD_NAME, val, 1, H5D_COMPACT);

    INFO(puts("Finished dopri853."));
    #ifdef SHOW_TIMING_INFO
    INFO(printf("time: %f seconds\n", mon.integration));
    h5_write_compact(H5_RUNTIME_STEPPER_NAME, &mon.integration, 1, H5D_COMPACT);
    #endif
    INFO(printf("steps: %d\n", dp.n_stp + 1));
