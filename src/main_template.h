...
#define GSL_OUTPUT_NUMBER       (__GSL_OUTPUT_NUMBER__)
...
struct output
{
    size_t dim;   ///< The length of the output on a single time slice.
    hsize_t id;   ///< The ID for the dataset in the h5 file.
    double *buf;  ///< A buffer for `WRITE_OUT_BUFFER_NUMBER` many time slices.
    double *tmp;  ///< The output values on the current time slice.
    int mode;     ///< An indicator whether to allocate the `tmp` field or not.
};
...
extern struct output rho_gw; ///< The output struct for \f$\rho_{gw}\f$
